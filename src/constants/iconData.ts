export const DOT_DATA: Record<
  string,
  { cx: number; cy: number; r?: number }[]
> = {
  pattern1: [
    { cx: 51.0874, cy: 6.20366 },
    { cx: 51.0874, cy: 28.6011 },
    { cx: 6.29055, cy: 50.9996 },
    { cx: 51.0874, cy: 51.0005 },
    { cx: 93.5688, cy: 51.1724 },
    { cx: 51.0874, cy: 73.398 },
    { cx: 28.689, cy: 73.398 },
    { cx: 71.2583, cy: 73.4839 },
    { cx: 51.0874, cy: 95.7964 },
  ],
  pattern2: [
    { cx: 17.4356, cy: 81.8572, r: 45 },
    { cx: 33.2715, cy: 66.0203, r: 45 },
    { cx: 49.1113, cy: 50.1814, r: 45 },
    { cx: 80.7871, cy: 50.1814, r: 45 },
    { cx: 80.7871, cy: 81.8581, r: 45 },
    { cx: 64.9492, cy: 34.3435, r: 45 },
    { cx: 50.7483, cy: 20.0205, r: 135 },
    { cx: 19.1936, cy: 20.0205, r: 135 },
    { cx: 80.7871, cy: 18.5056, r: 45 },
  ],
  pattern3: [
    { cx: 48.108, cy: 50.502 },
    { cx: 48.108, cy: 28.532 },
    { cx: 48.108, cy: 72.466 },
    { cx: 72.185, cy: 50.502 },
    { cx: 28.222, cy: 50.502 },
    { cx: 48.108, cy: 6.563 },
    { cx: 48.108, cy: 94.437 },
    { cx: 94.169, cy: 50.502 },
    { cx: 6.239, cy: 50.502 },
  ],
  plus: [
    { cx: 48.108, cy: 50.502 },
    { cx: 48.108, cy: 28.532 },
    { cx: 48.108, cy: 72.466 },
    { cx: 72.185, cy: 50.502 },
    { cx: 28.222, cy: 50.502 },
    { cx: 48.108, cy: 6.563 },
    { cx: 48.108, cy: 94.437 },
    { cx: 94.169, cy: 50.502 },
    { cx: 6.239, cy: 50.502 },
  ], // = 기존 pattern3와 동일한 좌표 (이미 플러스 모양이었음)
  search: [
    { cx: 32.65, cy: 59.72 },
    { cx: 10.69, cy: 45.9 },
    { cx: 7.78, cy: 20.13 },
    { cx: 26.31, cy: 1.71 },
    { cx: 51.9, cy: 4.69 },
    { cx: 65.71, cy: 26.65 },
    { cx: 57.13, cy: 51.14 },
    { cx: 68, cy: 77.76 },
    { cx: 85.14, cy: 100 },
  ], // 육각형 링(렌즈) 7개 + 손잡이 대각선 2개
  arrow: [
    { cx: 6.20366, cy: 51.0874 },
    { cx: 28.6011, cy: 51.0874 },
    { cx: 50.9996, cy: 6.29055 },
    { cx: 51.0005, cy: 51.0874 },
    { cx: 51.1724, cy: 93.5688 },
    { cx: 73.398, cy: 51.0874 },
    { cx: 73.398, cy: 28.689 },
    { cx: 73.4839, cy: 71.2583 },
    { cx: 95.7964, cy: 51.0874 },
  ], // 막대 4개 + 오른쪽 화살촉(위 2개, 아래 2개, 끝점 1개)
}
